...
final class FinalCorrectionTest extends TestCase
{
  ...
  public function testBatchSerializationCircularReference(): void
  {
    $data = [];
    $data["self"] = &$data;
    $responses = [Response::success(1, "fine"), Response::success(2, $data)];
    $encodedParts = [];
    foreach ($responses as $r) {
      $encodedParts[] = $r->toJson(
        JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE,
  ...
  public function testComputeRawItemCountSingleRequest(): void
  {
    $this->assertEquals(
      1,
      RateLimitManager::computeRawItemCount([
        "jsonrpc" => "2.0",
        "method" => "test",
      ]),
    );
  ...
  public function testFailOpenAllowsOnStorageError(): void
  {
    $limiter = new FileRateLimiter(
      maxRequests: 5,
      windowSeconds: 60,
      storagePath: "/nonexistent/path/that/cannot/be/created/" . uniqid(),
      failOpen: true,
    );
  ...
  public function testRateLimiterInterfaceCheckAndConsumeExists(): void
  {
    ...
    try {
      ...
      $this->assertInstanceOf(
        \Lumen\JsonRpc\RateLimit\RateLimiterInterface::class,
        $limiter,
      );
  ...
  public function testJwtLeewayAppliedToExpiredToken(): void
  {
    ...
    $response = $server->handle(
      $this->createRequest($body, "POST", ["Authorization" => "Bearer $token"]),
    );
    $data = json_decode($response->body, true);
  ...
  public function testGetHealthFiresBeforeRequestHook(): void
  {
    ...
    $server
      ->getHooks()
      ->register(HookPoint::BEFORE_REQUEST, function (array $ctx) use (
        &$fired,
      ) {
        $fired = true;
        return [];
  ...
  public function testGetHealthFiresOnResponseHook(): void
  {
    ...
    $server
      ->getHooks()
      ->register(HookPoint::ON_RESPONSE, function (array $ctx) use (&$fired) {
        $fired = true;
        return [];
      });
    $server->handle($this->createRequest("", "GET"));
  ...
  public function testGetHealthFiresAfterRequestHook(): void
  {
    ...
    $server
      ->getHooks()
      ->register(HookPoint::AFTER_REQUEST, function (array $ctx) use (&$fired) {
        $fired = true;
        return [];
      });
    $server->handle($this->createRequest("", "GET"));
  ...
  public function testGetHealthPassesHealthContext(): void
  {
    ...
    $server->handle($this->createRequest("", "GET"));
    $this->assertTrue(
      $captured["health"] ?? false,
      "GET health should set health=true in hook context",
    );
  }
  ...
  {
    $tmpFile =
      sys_get_temp_dir() . "/jsonrpc_test_config_valid_" . uniqid() . ".php";
  ...
  public function testRequestContextReceivesDecodedBody(): void
  {
    ...
    $server
      ->getHooks()
      ->register(HookPoint::BEFORE_HANDLER, function (array $ctx) use (
        &$capturedContext,
      ) {

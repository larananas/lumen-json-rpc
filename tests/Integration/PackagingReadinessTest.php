...
final class PackagingReadinessTest extends TestCase
{
    ...
    public function testComposerJsonIsValid(): void
    {
        ...
        $exitCode = 0;
        exec(
            "composer validate --strict --no-check-version 2>&1",
            $output,
            $exitCode,
        );
        $this->assertEquals(
            0,
            $exitCode,
            "composer.json must be valid: " . implode("\n", $output),
        );
    }
    ...
    public function testQualityDocsExplainCoverageDriverPrerequisites(): void
    {
        ...
        $docsSiteBuilder = file_get_contents(
            $this->rootDir . "/bin/build-docs-site.php",
        );

        $this->assertStringContainsString(
            "`composer qa` runs the standard local release gate",
            $readme,
        );
        $this->assertStringContainsString("PHPStan level 9", $readme);
        $this->assertStringContainsString(
            "`composer qa:max` extends that local gate with coverage threshold checks and mutation testing",
            $readme,
        );
        $this->assertStringContainsString(
            "`composer test:coverage` and `composer mutate` require a local coverage driver",
            $readme,
        );
        $this->assertStringNotContainsString("phpdbg", $readme);
        $this->assertStringContainsString(
            "`quality`, `tests`, `coverage`, and `mutation` jobs",
            $readme,
        );
        $this->assertStringContainsString(
            "does not commit `composer.lock`",
            $contributing,
        );
        ...
        $this->assertStringContainsString(
            "Xdebug coverage mode or PCOV",
            $contributing,
        );
        $this->assertStringNotContainsString("phpdbg", $contributing);
        $this->assertStringContainsString(
            "requires a local coverage driver",
            $docsSiteBuilder,
        );
        $this->assertStringContainsString(
            "same coverage-driver requirement",
            $docsSiteBuilder,
        );
        $this->assertStringContainsString(
            "XDEBUG_MODE=coverage",
        ...
        $this->assertStringNotContainsString(
            "run the command via <code>phpdbg</code>",
            $docsSiteBuilder,
        );
        $this->assertStringNotContainsString(
            "Xdebug coverage mode, PCOV, or phpdbg",
            $docsSiteBuilder,
        );
    }
    ...
    public function testQualityDocsAvoidClaimingExactLocalCiEquivalence(): void
    {
        ...
        $this->assertStringContainsString(
            "CI covers the same release areas",
            $readme,
        );
        $this->assertStringContainsString(
            "CI covers the same release areas as `qa:max`",
            $contributing,
        );
        $this->assertStringContainsString(
            "CI covers the same release areas across parallel jobs",
            $docsSiteBuilder,
        );
        $this->assertStringNotContainsString("match the full CI bar", $readme);
        $this->assertStringNotContainsString(
            "equivalent of `qa:max`",
            $contributing,
        );
        $this->assertStringNotContainsString(
    ...
    {
        $readme = file_get_contents($this->rootDir . "/README.md");
        $authDocs = file_get_contents(
            $this->rootDir . "/docs/authentication.md",
        );
        $contributing = file_get_contents($this->rootDir . "/CONTRIBUTING.md");

        $this->assertStringContainsString(
            "The stable public surface is centered on `JsonRpcServer`",
            $readme,
        );
        $this->assertStringContainsString(
            "`JsonRpcServer::getEngine()` is available as an escape hatch",
            $readme,
        );
        $this->assertStringContainsString(
            "stable server accessors like `getHooks()`, `getRegistry()`, and `getLogger()`",
            $readme,
        );
        $this->assertStringContainsString(
            "setRequestAuthenticator(new MyRequestAuthenticator())",
            $readme,
        );
        $this->assertStringContainsString("JsonRpcServer` API", $authDocs);
        $this->assertStringContainsString(
            "setRequestAuthenticator(new class implements RequestAuthenticatorInterface",
            $authDocs,
        );
        $this->assertStringContainsString(
            "stable server accessors like `getHooks()`, `getRegistry()`, and `getLogger()`",
        ...
        $this->assertStringContainsString(
            "tests/Integration/StablePublicApiTest.php",
            $contributing,
        );
        $this->assertStringContainsString(
            "patch releases may fix bugs, docs, tests, packaging, and internal implementation details",
            $readme,
        );
        $this->assertStringContainsString(
    ...
    public function testConfigurationReferenceMatchesPathAndHookDefaults(): void
    {
        $defaults = file_get_contents(
            $this->rootDir . "/src/Config/Defaults.php",
        );
        $configuration = file_get_contents(
            $this->rootDir . "/docs/configuration.md",
        );
    ...
    public function testCompressionDocsMatchTransportErrorSemantics(): void
    {
        ...
        $server = file_get_contents(
            $this->rootDir . "/src/Server/JsonRpcServer.php",
        );
        $corrections = file_get_contents(
            $this->rootDir . "/tests/Integration/CorrectionTest.php",
        );

        $this->assertStringContainsString(
            "Gzip decompression failures return `-32600 Invalid Request`",
            $security,
        );
        $this->assertStringNotContainsString("safe parse errors", $security);
        $this->assertStringContainsString(
            "Error::invalidRequest('Request body too large or decompression failed')",
            $server,
        );
        $this->assertStringContainsString(
            'assertEquals(-32600, $data[\'error\'][\'code\']);',
            $corrections,
        );
    }
    ...
    public function testReadmeDoesNotLinkToExportIgnoredDocsExamplesOrPolicies(): void
    {
        ...
        $this->assertDoesNotMatchRegularExpression(
            "/\]\((docs\/|examples\/|\.github\/|SECURITY\.md|CONTRIBUTING\.md)/",
            $readme,
        );
        $this->assertDoesNotMatchRegularExpression(
            '/<img[^>]+src="\.github\//',
    ...
    public function testDocsCallOutDiscoveryAndHookPolicies(): void
    {
        ...
        $architecture = file_get_contents(
            $this->rootDir . "/docs/architecture.md",
        );

        $this->assertStringContainsString(
            "auto-discovery only scans top-level handler files",
            $readme,
        );
        $this->assertStringContainsString(
            "Hook callbacks run inline.",
            $readme,
        );
        $this->assertStringContainsString(
            "Auto-discovery scans only the top-level PHP files",
            $configuration,
        );
        $this->assertStringContainsString(
            "Hook callbacks run inline with request processing.",
            $configuration,
        );
        $this->assertStringContainsString(
            "top-level handler files in configured paths",
            $architecture,
        );
        $this->assertStringContainsString(
            "hook exceptions are isolated, logged, and skipped",
            $architecture,
        );
    }
    ...
    {
        $readme = file_get_contents($this->rootDir . "/README.md");
        $authentication = file_get_contents(
            $this->rootDir . "/docs/authentication.md",
        );
        $configuration = file_get_contents(
            $this->rootDir . "/docs/configuration.md",
        ...
        $this->assertStringContainsString(
            "Generated JSON and OpenRPC docs also reuse these request schemas when available",
            $readme,
        );
        $this->assertStringContainsString(
            "descriptor metadata can also provide `resultSchema`",
            $readme,
        );
        $this->assertStringContainsString(
            "docblock tag such as `@result-schema",
            $readme,
        );
        $this->assertStringContainsString(
        ...
        $this->assertStringContainsString(
            "Prefer `password_hash` for production Basic auth credentials",
            $authentication,
        );
        $this->assertStringContainsString("`password_hash`", $configuration);
        $this->assertStringContainsString(
            "Runtime request schemas from `RpcSchemaProviderInterface` are also reused by the JSON and OpenRPC generators",
            $architecture,
        );
        $this->assertStringContainsString(
        ...
        $this->assertStringContainsString(
            "Runtime request schemas from <code>RpcSchemaProviderInterface</code>",
            $docsSiteBuilder,
        );
        $this->assertStringContainsString(
            "descriptor <code>resultSchema</code> metadata",
            $docsSiteBuilder,
        );
        $this->assertStringContainsString(
            "docblock <code>@result-schema</code> tags",
            $docsSiteBuilder,
        );
    }
    ...
    public function testAuthProtectionDefaultsAndDocsAreConsistent(): void
    {
        ...
        $basicExample = file_get_contents(
            $this->rootDir . "/examples/basic/config.php",
        );
        ...
        $this->assertStringContainsString(
            "exact methods or method prefixes",
            $readme,
        );
        $this->assertStringContainsString(
            "exact method names or trailing-separator prefixes",
            $authentication,
        );
        $this->assertStringContainsString(
    ...
    public function testDocsCallOutTransportStatusAndBatchOrderingPolicies(): void
    {
        ...
        $this->assertStringContainsString(
            "Mixed valid/invalid batches are not guaranteed to preserve original input order",
            $readme,
        );
        $this->assertStringContainsString("Allow: POST, GET", $readme);
        $this->assertStringContainsString(
            "representation-specific under gzip negotiation",
            $readme,
        );
        $this->assertStringContainsString(
            "JSON-RPC parse/protocol/application errors return HTTP `200`",
            $readme,
        );
        $this->assertStringContainsString(
            "Mixed valid/invalid batches are not guaranteed to preserve input order",
            $configuration,
        );
        $this->assertStringContainsString(
            "`GET /` is not advertised in the `Allow` header",
            $configuration,
        );
        $this->assertStringContainsString(
        ...
        $this->assertStringContainsString(
            "JSON-RPC parse/protocol/application errors still return HTTP 200",
            $configuration,
        );
        $this->assertStringContainsString(
            "patch releases may fix internals and packaging",
            $contributing,
        );
        $this->assertStringContainsString(
            "major releases may break the documented stable surface",
            $contributing,
        );
    }
    ...
    public function testValidationStrictDefaultIsLenientAcrossCodeAndDocs(): void
    {
        ...
        $authExample = file_get_contents(
            $this->rootDir . "/examples/auth/config.php",
        );

        $this->assertStringContainsString("'strict' => false", $defaults);
        $this->assertStringContainsString(
            "configBool('validation.strict', false)",
            $server,
        );
        $this->assertStringContainsString(
            "| `strict`         | `false` |",
    ...
    public function testRateLimitFailClosedDefaultIsConsistentAcrossCodeAndDocs(): void
    {
        ...
        $this->assertStringContainsString(
            "configBool('rate_limit.fail_open', false)",
            $server,
        );
        $this->assertStringContainsString(
            "| `fail_open`      | `false`",
        ...
        $this->assertStringContainsString(
            "By default, rate limiting is fail-closed",
            $readme,
        );
        $this->assertStringContainsString(
            "by default, requests are denied on storage failure",
            $security,
        );
        $this->assertStringContainsString(
            "'fail_open' => false",
    ...
    public function testOpenRpcServerUrlIsOptionalAcrossCodeAndDocs(): void
    {
        ...
        $generator = file_get_contents(
            $this->rootDir . "/src/Doc/OpenRpcGenerator.php",
        );
    ...
    public function testDocsDeploymentWorkflowPassesOptionalCustomDomainSecret(): void
    {
        $workflow = file_get_contents(
            $this->rootDir . "/.github/workflows/deploy-docs.yml",
        );
    ...
    public function testNoSilentJsonEncodeFallbacksInSource(): void
    {
        ...
        $this->assertEmpty(
            $violations,
            "No silent json_encode fallbacks allowed outside Response and LogFormatter: " .
                implode(", ", $violations),
        );
    }
    ...
    {
        $forbidden = [".phpcs-cache", "infection-log.txt", "coverage.xml"];
    ...
    public function testVerifyPackageScriptPasses(): void
    {
        ...
        exec(
            "php " .
                escapeshellarg($this->rootDir . "/bin/verify-package.php") .
                " 2>&1",
            $output,
            $exitCode,
        );
        $this->assertEquals(
            0,
            $exitCode,
            "verify-package.php must pass: " . implode("\n", $output),
        );
    }
    ...
    public function testComposerJsonAutoLoadPointsToSrc(): void
    {
        ...
        $this->assertSame(
            "src/",
            $json["autoload"]["psr-4"]["Lumen\\JsonRpc\\"],
        );
    }
